...
- **複数アカウント** — Gmail＋会社メール（IMAP）を同時接続。**統合受信箱**とアカウント別表示を切替。返信は元メールのアカウントから送信
...
- **スレッド/会話ビュー** — 同じ会話を時系列カードまたは**LINE風バブル**（自分=右）で表示。自分の返信も合流
- **連絡先（自動生成）** — やりとりから**手入力ゼロ**でアドレス帳を構築。人物ページで全履歴をチャット形式で振り返り（ミニCRMの種）
- **キーボード操作** — `j`/`k` 移動、`e` アーカイブ、`r` AI返信、`Shift+R` 普通の返信、`#`/`Backspace` ゴミ箱
